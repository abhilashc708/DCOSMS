        ...
        try {
              Class.forName("oracle.jdbc.driver.OracleDriver");
                ...
                Statement st=con.createStatement();
                  ...
                  String query="update admin set pass='"+pass1+"' where id='"+uid+"'and pass='"+pass+"'";
                  if(st.executeUpdate(query)>0){
                    
                        request.setAttribute("errormessage", "Change Password Succsessfully Completed");
         
                         getServletContext().getRequestDispatcher("/changeadminpass.jsp").forward(request, response);
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet adminpass</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet adminpass at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
             */
        } finally {            
